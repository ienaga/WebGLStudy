...
function main () {
    ...
    // 矩形 - 1
    var _x1 = x1 + Math.cos(angle1 * Math.PI / 180) * lw / 2;
    var _y1 = y1 + Math.sin(angle1 * Math.PI / 180) * lw / 2 * -1;
    var _x4 = x4 + Math.cos(angle1 * Math.PI / 180) * lw / 2;
    var _y4 = y4 + Math.sin(angle1 * Math.PI / 180) * lw / 2 * -1;

    var vertices = [
        (_x1-(w/2))/(w/2), -(_y1-(h/2))/(h/2),
        (x1-(w/2))/(w/2), -(y1-(h/2))/(h/2),
        (x4-(w/2))/(w/2), -(y4-(h/2))/(h/2),
        (_x4-(w/2))/(w/2), -(_y4-(h/2))/(h/2)
    ];

    var e0 = edge(_x1, _y1, _x4, _y4, h);
    var e1 = edge(x1, y1, _x1, _y1, h);
    var e3 = edge(_x4, _y4, x4, y4, h);
    ...
    var fSource = [
        ...
        "void main(void) {",
            ...
            "float alpha = min(a0, 1.0) * min(a1, a3);",
            "gl_FragColor = vec4("+ rgba.join(",") +") * alpha;",
        "}"
    ].join("\n");
    ...
    var vertices = [
        (x2-(w/2))/(w/2), -(y2-(h/2))/(h/2),
        (_x2-(w/2))/(w/2), -(_y2-(h/2))/(h/2),
        (_x3-(w/2))/(w/2), -(_y3-(h/2))/(h/2),
        (x3-(w/2))/(w/2), -(y3-(h/2))/(h/2)
    ];

    var e1 = edge(_x2, _y2, x2, y2, h);
    var e2 = edge(_x3, _y3, _x2, _y2, h);
    var e3 = edge(x3, y3, _x3, _y3, h);
    ...
    var fSource = [
        ...
        "void main(void) {",
            ...
            "float a3 = clamp(dot(vec3("+ e3.join(",") +"), pos), 0.0, 1.0);",
            "float alpha = min(1.0, a2) * min(a1, a3);",
            "gl_FragColor = vec4("+ rgba.join(",") +") * alpha;",
        "}"
    ].join("\n");
